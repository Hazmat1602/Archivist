...
def _get_or_create_category(db: Session, name: str, is_subcategory: bool, parent_id: int | None = None) -> Category:
    existing = db.query(Category).filter(Category.name == str(name).strip()).first()
    if existing:
        return existing

    category = Category(name=str(name).strip(), is_subcategory=is_subcategory, parent_id=parent_id)
    db.add(category)
    db.flush()
    return category
...
async def import_boxes(
    ...
    for _, row in df.iterrows():
        ...
        box = Box(code=box_code, name=box_name, created_date=created_date, expiry_date=expiry_date)
        db.add(box)
        db.flush()
